import { buildVoteEnvelope, verifyVoteEnvelope } from '../services/voteEnvelope.js';
import { persistVotes } from '../state/storage.js';
...
export function exportVotes({ res, state }) {
  ...
  return sendJson(res, 200, { entries: envelopes });
}
...
export async function gossipVotes({ req, res, state }) {
  ...
  return sendJson(res, 200, { added, total: state.votes.length });
}
