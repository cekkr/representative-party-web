...
export function renderProfileForm({
  ...
  const fields = providerFields.map((field) => renderProviderField(field, values[field.key])).join('\n');
...
function renderProviderField(field, value) {
  ...
  if (field.type === 'boolean') {
    ...
    return `
      <label class="field checkbox">
        <input type="checkbox" name="${FIELD_PREFIX}${key}" ${checked} />
  ...
  return `
    <label class="field">
      <span>${label}${field.required ? ' (required)' : ''}</span>
      <input name="${FIELD_PREFIX}${key}" type="${inputType}" value="${escapeHtml(inputValue)}" ${required} />
      ${description}
    </label>
  `;
}
