...
- Storage adapters: defaults live under `src/data/` (`ledger.json`, `sessions.json`, `peers.json`, `discussions.json`, `petitions.json`, `signatures.json`, `votes.json`, `delegations.json`, `notifications.json`, `topics.json`, `groups.json`, `group-policies.json`, `group-elections.json`, `actors.json`, `social-follows.json`, `social-posts.json`, `social-media.json`, `transactions.json`, `transaction-summaries.json`, `settings.json`, `meta.json`). Media binaries live under `src/data/media/` and are served through provider-gated endpoints. `adapters/memory.js` is ephemeral; `adapters/sql.js` uses SQLite (`DATA_SQLITE_URL|FILE`, needs `sqlite3`); `adapters/kv.js` stores everything in a single KV JSON file. `src/infra/persistence/storage.js` routes through `src/infra/persistence/store.js` to pick the adapter.
...
- `SOCIAL_MEDIA_MAX_BYTES` (max photo/video upload size, default 10MB), `SOCIAL_MEDIA_REPORT_THRESHOLD` (reports before auto-block, default 3)
