...
- Default JSON stores live under `src/data/` (`ledger.json`, `sessions.json`, `peers.json`, `discussions.json`, `petitions.json`, `signatures.json`, `votes.json`, `delegations.json`, `notifications.json`, `groups.json`, `group-policies.json`, `group-elections.json`, `actors.json`, `settings.json`, `meta.json`). `adapters/memory.js` offers an in-memory option for ephemeral runs/tests. `adapters/sql.js` implements a SQLite-backed key/value store (requires optional `sqlite3` dep); `adapters/kv.js` stores datasets in a single KV JSON file.
- `src/infra/persistence/migrations.js` normalizes schema versions (handles/roles, petition lifecycle, delegation/notifications, extensions, group elections, data topology, validationStatus on content/groups/delegations/notifications). Metadata lives in `meta.json`.
