...
- Main transactions registry: append-only log of sensitive operations (votes, petition drafts/status changes) with digest + issuer/mode/adapter, stored locally per provider (`transactions` store) for audit and cross-provider validation against signed envelopes.
