...
- Structure manager UI for provider-local fields and attributes is available; topic preferences + topic registry/breadcrumbs + gardener worker with scheduled refactors and admin rename/merge/split review are wired.

## Near-term next steps (Phase 2 focus)
- Extend topic gardener review to cover suggested anchor promotion/archival and deeper diff visualization for topic history.
