...
- Federation resilience: peers exchange ledger hashes and audit each other to quarantine toxic providers; gossip ingest rejects policy id/version mismatches and tracks peer health/quarantine; gossip sync treats module-disabled/gossip-disabled or missing optional endpoints as skipped (no peer-health penalty); when multiple providers belong to the same social-party ring, they should also prefer a shared client (or compatible clients) to keep UX and enforcement aligned across that ring.
...
- Topic gardener review: sync gardener operations into topic history and surface pending rename/merge/split suggestions in `/admin` for review/accept with a lightweight history diff view.
- Collaborative petitions: drafts support revision history, stage-aware editing (draft/discussion only), and UI cues for updates and version context.
...
- Gossip gating: when `DATA_MODE=centralized`, gossip ingest is disabled (403) to prevent unintended replication and `/admin` gossip controls are disabled; `hybrid` and `p2p` keep gossip enabled.
...
- Identity-based rate limiting (per handle/session with IP fallback) is configurable via `/admin` to keep posting flows CAPTCHA-free.
...
- Ops cues: `/admin` surfaces ledger hash, gossip ingest state, outbound/inbound gossip sync status, peer health reset actions, recent transactions, and inbound transaction summaries for audit snapshots; `/health` includes peer health summaries plus vote/transactions gossip added/updated counts for ops dashboards.
- Petition/vote scaffold: proposals persisted to JSON with per-role gating, collaborative revisions + version history, discussion notes, quorum → discussion (or admin-configured vote), and vote tallies; UI surfaces gate errors per role and stage cues.
