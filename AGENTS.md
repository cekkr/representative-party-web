...
- Federation resilience: peers exchange ledger hashes and audit each other to quarantine toxic providers; gossip ingest rejects policy id/version mismatches and tracks peer health/quarantine; gossip sync treats module-disabled/gossip-disabled peers as skipped (no peer-health penalty); when multiple providers belong to the same social-party ring, they should also prefer a shared client (or compatible clients) to keep UX and enforcement aligned across that ring.
